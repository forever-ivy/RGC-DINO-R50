...
CONVERT_ARGS=(
  ...
  --summary-path "$OUT_DIR/conversion_summary.json"
)
if [ -n "$CLASS_SCORE_THRESHOLDS" ]; then
  CONVERT_ARGS+=(--class-score-thresholds "$CLASS_SCORE_THRESHOLDS")
fi
if [ -n "$CLASS_ALLOCATION_CONFIG" ]; then
  CONVERT_ARGS+=(--class-allocation-config "$CLASS_ALLOCATION_CONFIG")
fi
if [ -n "$NMS_IOU_THRESHOLD" ]; then
  CONVERT_ARGS+=(--nms-iou-threshold "$NMS_IOU_THRESHOLD")
fi
PYTHONPATH=src python scripts/codetr_results_to_submission.py "${CONVERT_ARGS[@]}" \
  | tee "$OUT_DIR/conversion_summary.jsonl"

PREDICTION_OBJECTS=$(python - <<PY
import json
from pathlib import Path
summary_path = Path('$OUT_DIR/conversion_summary.json')
...
print(payload.get('prediction_objects', 0) if payload else 0)
PY
)
...
PROMOTE_ARGS=(
  ...
  --score-threshold "$SCORE_THRESHOLD"
  --candidate-score-threshold "$SCORE_THRESHOLD"
  --max-detections "$MAX_DETECTIONS"
  --conversion-summary-path "$OUT_DIR/conversion_summary.json"
  --config-path "$CONFIG"
  --split-manifest "$SPLIT_MANIFEST"
...
if [ -n "$IMAGE_MAX_SIDE" ]; then
  PROMOTE_ARGS+=(--image-max-side "$IMAGE_MAX_SIDE")
fi
if [ -n "${THRESHOLD_SWEEP_JSON:-}" ]; then
  PROMOTE_ARGS+=(--threshold-sweep-json "$THRESHOLD_SWEEP_JSON")
fi
if [ -n "${DIAGNOSTICS_PATH:-}" ]; then
  PROMOTE_ARGS+=(--diagnostics-path "$DIAGNOSTICS_PATH")
fi
if [ -n "${HARD_VAL_REPORT_PATH:-}" ]; then
  PROMOTE_ARGS+=(--hard-val-report-path "$HARD_VAL_REPORT_PATH")
fi
if [ -n "${HARD_VAL_STATUS:-}" ]; then
  PROMOTE_ARGS+=(--hard-val-status "$HARD_VAL_STATUS")
fi
if [ -n "${HARD_VAL_MAP_50_95:-}" ]; then
  PROMOTE_ARGS+=(--hard-val-map-50-95 "$HARD_VAL_MAP_50_95")
fi
